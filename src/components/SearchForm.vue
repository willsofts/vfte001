...
<div id="searchpanel" class="panel-body search-panel">
    <div class="row row-height">
        <div class="col-height col-md-3">
            <label>{{ labels.programids_label }}</label>
            <input type="text" v-model="localData.programid" class="form-control input-md" maxlength="20" />
        </div>
        <div class="col-height col-md-3">
            <label>{{ labels.prognames_label }}</label>
            <input type="text" v-model="localData.progname" class="form-control input-md" maxlength="50" />
        </div>
        <div class="col-height col-md-2">
            <label>{{ labels.progtypes_label }}</label>
            <select v-model="localData.progtype" class="form-control input-md">
              <option value=""></option>
...
const tableSettings = {
    ...
    columns: [
        {name: "programid", type: "STRING", sorter: "programid", label: "programid_headerlabel", css: "text-center" },
        {name: "progname", type: "STRING", sorter: "progname", label: "progname_headerlabel" },
        {name: "progtypedesc", type: "STRING", label: "progtypedesc_headerlabel" }
    ],        
    actions: [
...
export default {
  ...
  methods: {
    ...
    collecting(options,criterias) {
      ...
      startWaiting();
      $.ajax({
        url: getApiUrl()+"/api/sfte001/collect",
